...
export const withRedux = (PageComponent, { ssr = true } = {}): any => {
  ...
  if (ssr || PageComponent.getInitialProps) {
    WithRedux.getInitialProps = async (context): Promise<any> => {
    ...
    if (ssr || PageComponent.getServerSideProps) {
      WithRedux.getServerSideProps = async (context): Promise<any> => {
        // Get or Create the store with `undefined` as initialState
        // This allows you to set a custom default initialState
        const reduxStore = getOrInitializeStore({});
        ...
        // Run getInitialProps from HOCed PageComponent
        const pageProps =
          typeof PageComponent.getServerSideProps === "function"
